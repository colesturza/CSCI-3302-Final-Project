...
arms = []
armsNames = ['arm1', 'arm2', 'arm3', 'arm4', 'arm5']
for i in range(5):
    arms.append(robot.getMotor(armsNames[i]))
    arms[i].setPosition(float('inf'))
    arms[i].setVelocity(0.0)

print(arms[0].getMaxVelocity())

fingers = []
fingersNames = ['finger1', 'finger2']
for i in range(2):
    fingers.append(robot.getMotor(fingersNames[i]))
    fingers[i].setPosition(float('inf'))
    fingers[i].setVelocity(0.0)

print(fingers[0].getMaxVelocity())
